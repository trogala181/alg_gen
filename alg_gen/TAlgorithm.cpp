...
TAlg::TAlg(int pop_count, int max_generation_count, int progress_cap)
{
	...
	while (ending_cond==0) 
	{
		...
		cout << "Best value of candidate in " << i  << " generation: " << best_cand_val << "\n";
		...
		{
			int los = rand() % 100 + 1;
			int wybor1 = rand() % pop_count;
			int wybor2 = rand() % pop_count;
			if (los > 80)
			{
				wsk_pop_next->pops[i] = wsk_pop_next->pops[wybor1];
			}
			else if (los < 40)
			{
				wsk_pop_next->pops[i].crossing(wsk_pop_prev->pops[wybor1], wsk_pop_prev->pops[wybor1]);
			}
			else
			{
